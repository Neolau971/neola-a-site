  ...
  return (
    <div className="AppText">
      ...
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: '5rem' }}>
        ...
        <div>
          <List
            header={<div style={{ fontSize: '1rem' }}>{"Un brevet d'études professionnelles délivré par l'académie de Toulouse"}</div>}
            bordered
            dataSource={data0}
          ...
          <List
            header={<div style={{ fontSize: '1rem' }}>{"Une Maîtrise"}</div>}
            bordered
            dataSource={data1}
            renderItem={item => { return <List.Item>{item}</List.Item> }}
          />
          <div style={{ margin: '0.5rem' }} />
          <List
            header={<div style={{ fontSize: '1rem' }}>{"Un certificat d'aptitude professionnelle délivré par l'académie de Toulouse"}</div>}
            bordered
            dataSource={data2}
            renderItem={item => { return <List.Item>{item}</List.Item> }}
          />
          <div style={{ margin: '0.5rem' }} />
          <List
            header={<div style={{ fontSize: '1rem' }}>{"Des formations"}</div>}
            bordered
            dataSource={data3}
            renderItem={item => { return <List.Item>{item}</List.Item> }}
          />
